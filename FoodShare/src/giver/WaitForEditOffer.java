...
public class WaitForEditOffer extends Behaviour {
    boolean isDone = false;
    GiverAgent giver;

    WaitForEditOffer(GiverAgent agent) {
        giver = agent;
    }
    ...
    public void action() {
        ...
        MessageTemplate mtOntology = MessageTemplate.MatchOntology(OntologyNames.EDITING_OFFER_ONTOLOGY);
        ...
        if (message != null) {
            ...
            if (performative == ACLMessage.AGREE) {
                System.out.println("Oferta została zaktualizowana");
            } else if (performative == ACLMessage.REFUSE) {
                System.out.println("Oferta nie została zaktualizowana - weryfikacja nieudana");
            }
            isDone = true;
        } else {
